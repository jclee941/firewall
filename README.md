...
| `ConvertRequestsToSecuiBatch` | `requests` 결과의 `적용대상방화벽`을 장비별 행으로 분리해 `secui_batch` 양식으로 변환 |
| `ConvertRequestsToSecuiCli` | `requests` 결과의 `적용대상방화벽`을 장비별 `fw set srule ...` CLI 명령 초안으로 변환 |
...
| `vendor` | 제조사. `SECUI`이면 SECUI 배치/CLI 변환 대상 | `SECUI` |
| `enabled` | 사용 여부 | `TRUE` |
...
`MergeFirewallRequestFolder`와 `AnalyzeRequestRoutes`는 `requests` 시트에 아래 컬럼을 자동으로 채웁니다. `requests` 시트는 25개 컬럼을 2단 헤더 밴드(1행 그룹라벨 `출발지`/`목적지`를 IP+설명 위에 가로병합, 2행 리프 헤더)로 배치하고 데이터는 3행부터 시작합니다. 한 신청의 출발지IP×목적지IP×포트는 전체 조합으로 폭발(explode)되어 행마다 단일 값(1행=1방화벽룰)을 갖습니다. 같은 문서(요청부서/요청번호/제목)의 연속 행은 식별 셀이 세로병합됩니다(IP/포트는 병합 안 함). 표시 순서: `요청부서`, `요청번호`, `제목`, `원본파일`, `원본행`, `검증상태`, `적용대상방화벽`, `출발지IP`, `출발지설명`, `목적지IP`, `목적지설명`, `프로토콜`, `포트`, `방향`, `용도`, `시작일`, `종료일`, `비고`, `검증메시지`, `방화벽경로`, `출발Zone`, `목적Zone`, `Zone경로`, `매칭근거`, `요청폴더`.
...
| 표준 컬럼 | 허용 예시 |
|---|---|
| 출발지IP | 출발지 IP, 출발IP, Source, Src IP |
| 출발지 | 출발지명, 출발 |
| 목적지IP | 목적지 IP, 목적IP, Destination, Destiation, Dst IP |
| 목적지 | 목적지명, 목적 |
| 프로토콜 | Protocol, Proto |
| 포트 | Port, 목적지포트 |
| 용도 | 목적, Usage, Purpose |
...
`ConvertRequestsToSecuiCli`는 `requests`의 분석 결과 중 `firewalls.vendor=SECUI`이고 사용 중인 장비만 골라 `secui_cli` 시트에 장비별 `fw set srule ...` 명령 초안을 만듭니다. 공개된 BLUEMAX NGF 교육 자료 기준으로 정적 룰 적용 계열은 `fw set srule`입니다. 펌웨어별 옵션명이 다를 수 있으므로 장비 CLI에서 `fw set srule help`로 옵션을 확인한 뒤 붙여 넣으세요.

## 병합 셀 / 하위폴더 / 폴더명 파싱

### 세로 병합 셀

신청서에서 한 신청 건이 여러 행에 걸쳐 `출발지IP`/`목적지IP`/`포트` 같은 데이터 셀을 **세로 병합**한 양식도 지원합니다. 병합 영역의 값은 아래 행에도 적용되지만, 세로 병합의 연장선일 뿐 자체 입력값이 없는 행은 별도 신청 행으로 중복 처리하지 않습니다. 예를 들어 병합된 `포트` 셀 하나에 `80` 줄바꿈 `443`이 들어 있으면 논리 신청 행은 1개로 읽고, 결과는 `80`과 `443` 두 정책 행으로 한 번만 폭발(explode)됩니다. 반대로 같은 병합 IP를 공유하더라도 각 행의 `포트`/`용도` 등 신청 필드에 고유 값이 있으면 각 행을 별도 신청으로 유지합니다. (데이터 셀은 병합 좌상단 값을 `MergeArea`로 읽고, 헤더 행은 좌상단만 인식합니다.) 제목/그룹 헤더가 헤더 위에 병합되어 있어도 헤더 내용 기반 인식으로 실제 헤더 행을 찾아 우회합니다.
