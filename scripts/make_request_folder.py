...
Each .xlsx matches what the VBA parser reads:
  ...
  - columns: 대상방화벽 출발지IP 출발지 목적지IP 목적지 프로토콜 포트 방향 용도 시작일 종료일 비고
...
REQUESTS = [
    (
        ...
        "신청서_웹서비스.xlsx",
        [
            [1, "SECUI-FW-01", "10.10.10.5", "업무PC", "172.16.1.10", "업무시스템",
             "TCP", "443", "OUT", "HTTPS 업무 연동", "2026-01-01", "2026-12-31", "정기 신청"],
            [2, "SECUI-FW-01;SECUI-FW-02", "10.10.20.0/24", "업무PC대역", "10.20.20.5", "DMZ서버",
    ...
    (
        ...
        "방화벽신청.xlsx",
        [
            [1, "SECUI-FW-03", "10.10.30.10", "운영서버", "8.8.8.8", "외부DNS",
    ...
    (
        ...
        "신청.xlsx",
        [
            [1, "SECUI-FW-01", "10.10.40.7", "관리PC", "172.16.1.20", "관리시스템",
             "TCP", "22", "OUT", "SSH 관리 접근", "2026-03-01", "2026-09-30", "한시 신청"],
            [2, "SECUI-FW-01;SECUI-FW-02", "10.10.50.1;10.10.50.2", "관리PC군", "10.20.20.9", "DMZ관리",
...
3) Excel 에서 매크로 MergeFirewallRequestFolder 실행(Alt+F8) -> 신청서 통합.

신청서 .xlsx 양식
- 첫 시트에 'No'(또는 '번호') 가 있는 행이 헤더 행입니다(이 폴더 파일은 B열에 No).
- 필요한 컬럼: 대상방화벽 출발지IP 출발지 목적지IP 목적지 프로토콜 포트 방향 용도 시작일 종료일 비고
...
대상방화벽은 SECUI CLI 생성 기준입니다. 여러 장비는 ; 로 구분합니다.
"""
