...
`requests`는 최종 신청 목록에 필요한 14개 컬럼만 표시합니다: `요청부서`, `요청번호`, `대상방화벽`, `출발지`, `출발지설명`, `목적지`, `목적지설명`, `프로토콜`, `포트`, `방향`, `용도`, `시작일`, `종료일`, `비고`. 원본파일/원본행/요청폴더/제목은 숨김 `_request_tracking` 시트에 보관하고, 경로탐색 관련 컬럼은 `route_results`로 분리합니다.
