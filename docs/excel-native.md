...
8. `settings` 시트의 `parse_targets` 값을 확인합니다. 기본값은 `출발지IP;목적지IP`입니다.
...
| `zone_path` | zone 그래프 BFS 최단경로의 zone 열 |
...
목적지IP만 기준으로 통합하려면 `parse_targets`를 `목적지IP`로 둡니다. 출발지IP와 목적지IP를 함께 보려면 `출발지IP;목적지IP`처럼 세미콜론으로 구분합니다.
