...
`secui_batch`와 `secui_cli`는 `requests.대상방화벽`을 장비별로 분리해 생성합니다. `firewalls.vendor`가 `SECUI`이고 사용 중인 장비만 포함합니다. 라우팅 자동 탐색은 추후 기능이며, 현재 CLI 생성은 사용자가 입력한 `대상방화벽`을 기준으로 합니다. 통합 문서를 열면 `request_folder`가 존재할 때 신청서 통합 후 두 출력 시트를 자동 갱신합니다.
