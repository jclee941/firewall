...
`requests`는 운영자가 보는 14개 컬럼만 고정으로 사용합니다. 1행은 출발지/목적지 그룹 라벨, 2행은 실제 헤더, 데이터는 3행부터입니다. 원본파일/원본행/요청폴더/제목 추적값은 숨김 `_request_tracking` 시트에 저장되고, 경로탐색 결과는 `route_results`에서 확인합니다.
...
`route_results`는 `AnalyzeRequestRoutes`가 `requests`의 신청값과 숨김 `_request_tracking`의 원본 추적값을 기준으로 다시 생성하는 표시용 시트입니다. 주요 컬럼은 `요청부서`, `요청번호`, `출발지`, `목적지`, `방향`, `대상방화벽`, `검증상태`, `검증메시지`, `방화벽경로`, `출발매칭대역`, `목적매칭대역`, `대역경로`, `매칭근거`, `원본파일`, `원본행`입니다.
