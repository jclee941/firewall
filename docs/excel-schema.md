...
| 시트 | 역할 |
| --- | --- |
| `requests` | 최종 신청 목록과 내부 CLI 생성 입력 |
| `firewalls` | 방화벽 장비 정의 |
...
`requests`는 내부적으로 25개 컬럼을 고정으로 사용합니다. 1행은 출발지/목적지 그룹 라벨, 2행은 실제 헤더, 데이터는 3행부터입니다. 화면에는 최종 신청 목록에 필요한 `요청부서`, `요청번호`, `출발지`, `출발지설명`, `목적지`, `목적지설명`, `프로토콜`, `포트`, `시작일`, `종료일`, `비고`만 표시합니다. 원본파일/원본행과 경로탐색 관련 컬럼은 숨기고, 경로 결과는 `route_results`에서 확인합니다.
...
`route_results`는 `AnalyzeRequestRoutes`가 `requests`의 숨김 경로 컬럼을 기준으로 다시 생성하는 표시용 시트입니다. 주요 컬럼은 `요청부서`, `요청번호`, `출발지`, `목적지`, `방향`, `대상방화벽`, `검증상태`, `검증메시지`, `방화벽경로`, `출발매칭대역`, `목적매칭대역`, `대역경로`, `매칭근거`, `원본파일`, `원본행`입니다.
...
`secui_cli`는 `requests.대상방화벽`을 장비별로 분리해 생성합니다. `firewalls.vendor`가 `SECUI`이고 사용 중인 장비만 포함합니다. `대상방화벽`이 비어 있으면 신청서 파싱 후 `firewall_ranges` 경로탐색 결과로 자동 산출한 값을 사용합니다. 통합 문서를 열면 `request_folder`가 존재할 때 신청서 통합, 경로탐색, CLI 출력 갱신을 순서대로 실행합니다.
