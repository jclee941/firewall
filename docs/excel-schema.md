...
`secui_cli`는 `requests.대상방화벽`을 장비별로 분리해 생성합니다. `firewalls.vendor`가 `SECUI`이고 사용 중인 장비만 포함합니다. `대상방화벽`이 비어 있으면 신청서 파싱 후 `firewall_ranges` 경로탐색 결과로 자동 산출한 값을 사용합니다. 통합 문서를 열거나 `F9`를 누르면 신청서 통합, 경로탐색, CLI 출력 갱신을 순서대로 실행합니다.
